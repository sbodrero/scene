<?php
	...
	*/
	...
	function print_item_size($item_size, $item_row_size = '0', $addition_class='', $html_row_tag = 'div', $html_col_tag = 'div'){
		...
		if($item_row_size == 0.5){
			$item_row_size = $gdl_item_size;
			echo '<div class="clear"></div>';
			echo '</div>'; // close row
			echo '<' . $html_row_tag . ' class="row1">'; // open new row
		}
	...
	function print_page_header( $header ){
		...
		echo '<h1 class="gdl-header-title">' . $header . '</h1>';
		...
		if( !empty($rating) && $rating != 'none' ){
			...
			for( $i=0; $i<5; $i++ ){
				if( $rating - $i == 0.5 ){
					...
					echo '<span class="star none"></span>';
				}
			}
			echo '</div>';
		}
	...
	function print_column_service($item_xml){
		...
		echo '<div class="column-service-wrapper ' . $column_service_style . '">';
		...
		echo '<div class="column-service-caption">' . $column_service_caption . '</div>';
	...
	function print_post_slider_item($item_xml){
		...
		query_posts(array('post_type'=>'event', 'category_name'=>$category, 'posts_per_page'=>$num_fetch, 'meta_key' => '_thumbnail_id' ));
	...
	function print_top_post_slider( $category, $height, $thumbnail, $num_excerpt, $num_fetch ){
		...
		echo '<div class="flexslider gdl-slider ' . $carousel_class . '" >';
		echo '<ul class="slides">';
		query_posts(array('post_type'=>'event', 'category_name'=>$category, 'posts_per_page'=>$num_fetch, 'meta_key' => '_thumbnail_id' ));
		while( have_posts() ){ the_post();

			$thumbnail_id = get_post_thumbnail_id(get_the_ID()) !='' ? get_post_thumbnail_id(get_the_ID()) : get_placeholder_slider_image();
			...
			echo '<h2 class="gdl-slider-title">' . get_the_title() . '</h2>';
			echo '<div class="gdl-slider-caption">';
			echo gdl_get_excerpt($num_excerpt);
			echo '<a href="' . get_permalink() . '" class="gdl-slider-read-more">' . $translator_read_more . '</a>';
			echo '</div>';
			echo '<div class="gdl-slider-info">';

			echo '<div class="blog-date-wrapper">';
			echo '<a href="' . get_permalink() . '" class="gdl-slider-read-more">'; //remplace la ligne d'en-dessous
		...
		if( $thumbnail ){
			...
			echo '<ul class="slides">';
			foreach($thumbnail_arr as $thumbnail_id){
				$image_url = wp_get_attachment_image_src($thumbnail_id, $thumbnail_width . 'x' . $thumbnail_height );
				$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
	...
	function print_stunning_text($item_xml){
		...
		if( !empty($button_title2) && !empty($button_link2) ){
			echo '<a class="stunning-text-button" href="' . $button_link2 . '">';
			echo $button_title2;
			echo '</a>';
		}
