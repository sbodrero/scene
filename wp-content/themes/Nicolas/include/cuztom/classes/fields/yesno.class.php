...
class Cuztom_Field_Yesno extends Cuztom_Field
{
	...
	function _output( $value )
	{
		...
		$output .= '<div class="cuztom-checkbox-wrap">';
			...
			$output .= sprintf( '<label class="cuztom-label" for="%s_yes">%s</label>', $this->id, __( 'Professionnel', 'cuztom' ) );
			$output .= '<br />';
			...
			$output .= sprintf( '<label class="cuztom-label" for="%s_no">%s</label>', $this->id, __( 'Non professionnel', 'cuztom' ) );
		$output .= '</div>';
