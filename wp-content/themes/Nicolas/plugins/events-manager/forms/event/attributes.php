...
<div id="event-attributes">
	<?php if( !empty($attributes['names']) && count( $attributes['names'] ) > 0 ) : ?>
						...
						<td>
							...
							<select name="em_attributes[<?php echo $name ?>]">
								<?php foreach($attributes['values'][$name] as $attribute_val): ?>
									...
									<?php endif; ?>
								<?php endforeach; ?>									<?php /*										query_posts( array( 'post_type' => 'artiste' ) );											while ( have_posts() ) :  the_post();?>										<option value="<?php the_title() ?>"><?php echo $attributes['values'][$name] = the_title();  ?></option>									<?php	endwhile; wp_reset_query();?>											<?php	echo   $attributes['values'];													echo $attributes['values'][$name][0];													*/ ?> 										
							</select>
							<?php else: ?>
		...
		<p><em><?php _e("If you see any attributes under here, that means they're not used in Events Manager formats. To add them, you need to add the custom attribute again to a formatting option in the settings page. To remove any of these depreciated attributes, give it a blank value and save.",'dbem') ?></em></p>
		<table class="form-table">
			<thead>
				<tr valign="top">
					<td><strong>Attribute Name</strong></td>
