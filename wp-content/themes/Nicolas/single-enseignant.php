		...
		<?php
			...
			if ( have_posts() ){
				while (have_posts()){
					...
					$image =  wp_get_attachment_image(get_post_meta($post->ID, _enseignant_info_image, true), full);
		...
		<div class="blog-media-wrapper gdl-image" style="height:100px;"></div>
			...
			<?php
					...
					the_content(); 
						...
						<li class="gdl-divider">
							<h2 class="toggle-box-head title-color gdl-title">Les ateliers</h2>
							<div class="toggle-box-content active">
...
<a href="<?php the_permalink(); ?>">
						...
						echo wp_trim_words(get_the_excerpt(),15);
						  ...
						  <div>
							...
							else {the_author(); echo ' n\'a pas encore présenté d\'atelier';} ?>
							</div>
						</li>
						<li class="gdl-divider">
						<h2 class="toggle-box-head title-color gdl-title">Stages et événements</h2>
						<div class="toggle-box-content active">
<?php 
	$hauteur = get_the_author_meta( 'ID');
	$query = 	array(
					'author' => $hauteur,
					'post_type' => 'event',
					'meta_key' => '_start_ts',    //pour supprimer les événements passés
					'meta_value' => current_time('timestamp'), //pour supprimer les événements passés
					'meta_value_num' => current_time('timestamp'), //pour supprimer les événements passés
					'meta_compare' => '>=',
					'orderby' => 'date',
					//'category_name' => 'Stage', 'Spectacle',
					'posts_per_page' => -1			
		); 
			$requete = new WP_Query($query);
				if ( $requete->have_posts() ) { 
			...
			<div class="six columns gdl-blog-grid">
				<div class="blog-content-wrapper">
					...
					<div class="blog-context-wrapper">
						...
						<li class="gdl-divider">
							<h2 class="toggle-box-head title-color gdl-title">Dossiers de formation </h2>
							<div class="toggle-box-content active">
<?php	
	
	$dossiers =  get_post_meta($post->ID, '_enseignant_info_dossier_formation', true);
		if ($dossiers != "") {
								?>
											
	 			<a href="<?php echo $dossiers ?>" target="blank" alt ="Le dossier de formation de <?php echo the_title(); ?>"> <?php echo '<img src="'.get_icon_for_attachment($dossiers->ID).'" />   '?> </a> : le dossier de formation de <?php the_author(); ?> <br/>

								<?php 	}	

						else {the_author(); echo ' n\'a pas encore téléchargé de dossier de formation';}
...
<div class="four columns gdl-right-sidebar"><div class="sidebar-wrapper"><div class="custom-sidebar">
				...
				<?php
				global $post;
				$resp_legal =  get_post_meta($post->ID, _enseignant_info_resp_legal, true);
				$agitateur =  get_post_meta($post->ID, _enseignant_info_agit_artistique, true);
				$compagnie =  get_post_meta($post->ID, _enseignant_info_compagnie, true);
				$public =  get_post_meta($post->ID, _enseignant_info_public, true);
				$telephone =  get_post_meta($post->ID, _enseignant_info_telephone, true);
				$image =  wp_get_attachment_image(get_post_meta($post->ID, _enseignant_info_image, true), full);
				$mail =  get_post_meta($post->ID, _enseignant_info_email, true);
				$facebook =  get_post_meta($post->ID, _enseignant_reseauxsociaux_facebook, true);
				$myspace =  get_post_meta($post->ID, _enseignant_reseauxsociaux_myspace, true);
				$googeplus =  get_post_meta($post->ID, _enseignant_reseauxsociaux_gplus, true);
				$linkedin =  get_post_meta($post->ID, _enseignant_reseauxsociaux_linkedin, true);
				$video =  get_post_meta($post->ID, _enseignant_reseauxsociaux_video, true);
				$site =  get_post_meta($post->ID, _enseignant_info_siteweb, true);
				$adresse =  get_post_meta($post->ID, _enseignant_info_adresse_rue, true). ' ' .get_post_meta($post->ID, _enseignant_info_adresse_ville, true). ' '.get_post_meta($post->ID, _enseignant_info_adresse_cp, true);
				$prooupas =  get_post_meta($post->ID, _enseignant_info_statut, true);
				$partenaires_insitutionnels = get_post_meta($post->ID, '_enseignant_info_partenaires_insitutionnels', true);
				$url = "http://www.desceneenscene.fr/ng-19-02/wp-content/themes/desceneenscene/images"; 
				?>
				<?php
...
?>
			...
			<li><img src="<?php echo $url; ?>/mail.png" title ="">
					...
					<li style="background:none !important;"> <!-- les réseaux sociaux-->

					<!--?php if(($facebook != '') && ($myspace != '') && ($googeplus != '') && ($linkedin != '') && ($video != '')){ echo "Aucun réseau social renseigné";}?-->
					...
					<li>
						<table>
							<tr>
								<td style="width:10%"><img src="<?php echo $url; ?>/web.png" title =""></td>
